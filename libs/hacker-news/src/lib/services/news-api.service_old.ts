  ...
  fetchNewsFeed() {
    return this.newsOutput$.pipe(
      ...
      switchMap((params) => {
        ...
        return this.httpClient.get<INewsFeed>(this.config.NEWS_API_URL, {
          params,
        });
      }),
      tap((response) => {
        ...
        const noOfPagesPaginator = Math.ceil(
          totalResults / this.config.PAGE_SIZE
        );
