...
import { CalculatorConfig } from "src/app/calculator/constants/calculator.const";
import { ICalculatorLayout } from "src/app/calculator/types/calculator.interface";
...
export class CalculatorComponent implements OnInit {
  ...
  private storeOperationHistoryInCurrentSession(currentOutput: number): void {
    let previousOperations = JSON.parse(
      sessionStorage.getItem("operationHistory")
    );
    let currentOperationHistory = {
      operation: `${this.operation} ${currentOutput}`,
      result: this.output,
    };
